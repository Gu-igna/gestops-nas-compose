...
def login():
    ...
    if usuario.validate_pass(data.get("password")):
        ...
        return {"access_token": access_token, "usuario": usuario.to_json_short()}, 200
    else:
...
def register():
    ...
    if db.session.query(UsuarioModel.id).filter(UsuarioModel.email == usuario.email).scalar():
        return {"error": "Email duplicado"}, 409
...
def update_password():
    ...
    if not new_password:
        return {'error': 'La nueva contraseña es obligatoria'}, 400
    ...
    try:
        ...
        if reset_token:
            ...
            if current_password_hash_part != password_hash_part_from_token:
                return {'error': 'Token inválido o ya utilizado'}, 400
        ...
        # CASO 2: Usuario logueado quiere cambiar contraseña
            ...
            if not current_password:
                return {'error': 'La contraseña actual es requerida'}, 400
